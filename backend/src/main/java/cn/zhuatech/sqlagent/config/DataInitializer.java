...
@Configuration public class DataInitializer {
 ...
 resources.saveAll(List.of(new ResourceRegister("DW-PROD","经营分析数据仓库",u1,ResourceRegister.Status.RUNNING,99),new ResourceRegister("CRM-RO","CRM 只读副本",u1,ResourceRegister.Status.RUNNING,97),new ResourceRegister("META-01","企业元数据与指标库",u3,ResourceRegister.Status.IDLE,95)));
 reviews.saveAll(List.of(new ReviewRecord("RV-260817-032",t1,"敏感字段复核",8,1,ReviewRecord.Result.PENDING,"许澄"),new ReviewRecord("RV-260817-027",t2,"执行计划",6,0,ReviewRecord.Result.PASSED,"林峤"),new ReviewRecord("RV-260817-018",t4,"写操作拦截",5,3,ReviewRecord.Result.FAILED,"许澄")));
 String demo=encoder.encode("Demo@2026");users.saveAll(List.of(new UserAccount("operator",demo,"林峤",UserAccount.Role.DOMAIN_USER,"ANALYTICS"),new UserAccount("planner",demo,"许澄",UserAccount.Role.DOMAIN_OPERATOR,null),new UserAccount("quality",demo,"沈言",UserAccount.Role.QUALITY,null),new UserAccount("admin",encoder.encode("ZhuaTech@2026"),"系统管理员",UserAccount.Role.ADMIN,null)));};}
}
